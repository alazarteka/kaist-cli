...
- `kaist-v0.2.0-linux-x86_64-musl.tar.gz`
- `checksums.txt`
...
2. Builds a musl-linked Linux x86_64 standalone bundle inside Alpine for headless Linux hosts.
3. Calls `scripts/build_release_bundle.sh` to package one managed bundle archive per target.
4. Generates a combined `checksums.txt` covering all release archives.
5. Uploads all archives plus `checksums.txt` to the GitHub release for the tag.
...
- Linux installs auto-detect `linux-x86_64-musl`
- non-musl Linux is not supported by the published Linux release in this tranche
...
- Linux self-update currently supports only the published musl target
