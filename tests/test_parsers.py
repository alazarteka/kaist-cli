...
from kaist_cli.v2.klms.assignments import _extract_assignment_rows_from_calendar_data
from kaist_cli.v2.klms.courses import _discover_courses_from_dashboard, _extract_current_term_from_dashboard
from kaist_cli.v2.klms.files import _is_video_filename, _is_video_url, _material_kind_from_module
from kaist_cli.v2.klms.notices import _extract_pagination_pages, _parse_notice_detail_from_html, _parse_notice_items_from_soup
...
def test_dashboard_parser_fixture() -> None:
    ...
    rows = _discover_courses_from_dashboard(html, base_url="https://klms.kaist.ac.kr")
    assert len(rows) == 2
    assert rows[0].id == "180871"

    term = _extract_current_term_from_dashboard(html)
    assert term == "2026 Spring"


def test_notice_parser_fixture() -> None:
    html = (FIXTURES / "notices_sample.html").read_text(encoding="utf-8")
    rows = _parse_notice_items_from_soup(
    ...
    pages = _extract_pagination_pages(BeautifulSoup(html, "html.parser"))
    assert pages == [0, 2]
...
def test_notice_detail_parser_fixture() -> None:
    ...
    assert "exam-room-map.pdf" in attachments[0]["url"]
...
def test_assignment_api_shape_parser_fixture() -> None:
    ...
    rows = _extract_assignment_rows_from_calendar_data(payload, base_url="https://klms.kaist.ac.kr", auth_mode="profile")
    assert len(rows) == 1
...
def test_file_parser_helpers() -> None:
    assert _is_video_filename("lecture.mp4") is True
    assert _is_video_url("https://cdn.example.com/stream/hls") is True
    assert _material_kind_from_module("resource") == "file"
